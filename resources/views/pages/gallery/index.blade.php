...
@section('main')
    <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        <div class="max-w-screen-xl px-4 py-8">
            <!-- Start coding here -->
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">List of Gallery Product</h2>
            <div class="bg-white pb-8 dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center" action="{{ route('product/gallery') }}" method="GET">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @if ($galleries->isNotEmpty())
                                @foreach ($galleries as $item)
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <td class="px-4 py-3">
                                            <img class="h-10 rounded" src="{{ $item->url }}" alt="Gallery Photo">
                                        </td>
                                        <td class="px-4 py-3">{{ $item->product->name }}</td>
                                        <td class="px-4 py-3">{{ $item->created_at }}</td>
                                        <td class="px-4 py-3 text-end">
                                            <a href="/product/gallery/{{ $item->id }}"
                            ...
                            @else
                                <tr class="border-b dark:border-gray-700">
                                    <td colspan="5" class="px-4 py-3">Not found gallery.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
    ...
    @include('pages.gallery.modals.add')

    @include('pages.gallery.modals.delete')
@endsection
